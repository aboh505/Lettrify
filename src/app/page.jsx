  ...
  return (
    <motion.div className="p-4 bg-gradient-to-br from-slate-100 via-white to-slate-200 min-h-screen">
      ...
      <div className="flex justify-center gap-4 mb-4">
        <button
          onClick={() => setMode("cv")}
          className={`px-6 py-2 rounded-full shadow-md transition-all duration-200 ${mode === "cv" ? "bg-blue-600 text-white transform scale-105" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}
        >
          CV
        </button>
        <button
          onClick={() => setMode("letter")}
          className={`px-6 py-2 rounded-full shadow-md transition-all duration-200 ${mode === "letter" ? "bg-blue-600 text-white transform scale-105" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}
        >
          Lettre
        </button>
      </div>
      ...
      >
        {mode === "cv" ? (
          <>
            <CVForm data={cvData} setData={setCvData} type={cvType} />
            <CVPreview data={cvData} template={template} type={cvType} />
          </>
        ) : (
          <>
            <LetterForm data={letterData} setData={setLetterData} autoFill={cvData} type={letterType} />
            <LetterPreview data={letterData} template={template} type={letterType} />
          </>
        )}
      </motion.div>
